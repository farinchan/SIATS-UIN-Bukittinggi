...
class ModulAlumni extends CI_Controller
{
    ...
    function logupdatealumni()
    {
        $this->session->set_flashdata('location', "modul alumni");
        $this->Model_Alumni->mark_all_as_read();

        $data = array(
            'log_update_alumni' => $this->Model_Alumni->getlogupdatealumni()
        );

        $this->load->view('admin/modulpengguna/logpengguna.php', $data, TRUE);
        $this->load->view('admin/modulpengguna/modul_logpengguna.php', $data, FALSE);
    }

    public function listLogupdatealumni()
    {

        $list = $this->Model_Alumni->getlogupdatealumni();
        $data = array();
        $no = $_POST['start'];
    ...
    function cetakalumni()
    {
        ...
        if (empty($nisn) || $this->Model_Alumni->getAlumni($nisn)->num_rows() == 0) {
            ...
            $data = array(
                'data_profil' => $this->Model_Alumni->getAlumni($nisn)->row(),
            ...
            $data['title'] = "Cetak Data Alumni";
            $file_pdf = $data['title'];
            $paper = 'A4';
            $orientation = "potrait";
            $html = $this->load->view('admin/modulalumni/cetakalumni_pdf', $data, true);
    ...
    function cetaklaporan()
    {
        ...
        $data['title'] = "LAPORAN REKAPITULASI DATA ALUMNI";
        $file_pdf = $data['title'];
        $paper = 'A4';
        $orientation = "potrait";
        $html = $this->load->view('admin/modulalumni/cetaksemuaalumni_pdf', $data, true);
        $this->pdfgenerator->generate($html, $file_pdf, $paper, $orientation);
    }
    ...
    public function editberita()
    {
        ...
        if ($this->Model_Alumni->getAlumni($nisn)->num_rows() == 0) {
