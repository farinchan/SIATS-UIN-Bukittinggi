...
$('#paginationadmin').on('click','a',function(e){
    e.preventDefault(); 
    var pageno = $(this).attr('data-ci-pagination-page');
    loadPaginationadmin(pageno);
});

loadPaginationadmin(0);

function loadPaginationadmin(pagno){
    $.ajax({
        url: 'listberitadmin/'+pagno,
        type: 'GET',
        dataType: 'json',
        success: function(response){
        $('#paginationadmin').html(response.pagination);
        createListadmin(response.result,response.row);
        }
    });
}

function createListadmin(result,sno){
    sno = Number(sno);
    $('#databeritaadmin').empty();
    for(index in result){
        var judul_berita = result[index].judul_berita;
        var isi_berita = result[index].isi_berita;
        var gambar_berita = result[index].gambar_berita;
