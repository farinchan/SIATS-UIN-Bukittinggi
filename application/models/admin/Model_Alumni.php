...
class Model_Alumni extends CI_Model
{
    ...
    public function get_listalumni_query($isi_tracer, $tahun_lulus, $kode_prodi, $nama)
    {
        ...
        $this->db->select('alumni.*, prodi.*, tracer_univ.p1');
        $this->db->from('alumni');
        $this->db->join('prodi', 'alumni.kode_prodi = prodi.kode', 'inner');
        $this->db->join('tracer_univ', 'alumni.nisn = tracer_univ.nisn', 'left');

        if ($nama !=  "0") {
            $this->db->like('nama_alumni', $nama);
        }

        if ($isi_tracer == 1) {
            $this->db->where('tracer_univ.p1 !=', null);
        }
    ...
    public function getlistalumni($isi_tracer, $tahun_lulus, $kode_prodi, $nama)
    {

        $this->get_listalumni_query($isi_tracer, $tahun_lulus, $kode_prodi, $nama);
        if ($_POST['length'] != -1) {
    ...
    public function count_filtered_alumni($tracer, $lulus, $kode_prodi, $nama)
    {

        $this->get_listalumni_query($tracer, $lulus, $kode_prodi, $nama);
